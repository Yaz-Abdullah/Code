...
const Question = ({ type, mongoUserId, questionDetails }: Props) => {
  ...
  const watchedExplanation = useWatch({ control: form.control, name: 'explanation' }) || '';
  ...
  const handleApplySuggestedTitle = (value: string) => {
    form.setValue('title', value, { shouldDirty: true, shouldValidate: true });
  }

  const handleApplySuggestedTags = (suggestedTags: string[]) => {
    const existingTags = new Set(watchedTags.map((tag: string) => tag.trim().toLowerCase()));
    const mergedTags = [...form.getValues('tags')];

    suggestedTags.forEach((tag) => {
      const normalized = tag.trim().toLowerCase();
      if (normalized && !existingTags.has(normalized) && mergedTags.length < 3) {
        mergedTags.push(normalized);
        existingTags.add(normalized);
      }
    });

    form.setValue('tags', mergedTags, { shouldDirty: true, shouldValidate: true });
  }
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="flex w-full flex-col gap-10">
        ...
        <QuestionSuggestions
          ...
          tags={watchedTags}
          onApplyTitle={handleApplySuggestedTitle}
